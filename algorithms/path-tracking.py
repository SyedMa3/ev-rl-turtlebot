...
class optAgent(Node):
    ...
    def process(self):
        ...
        for episode in range(1, 10000):
            ...
            # print((z_k - z))
            if episode == 1:
                phi = np.kron((z_k - 0.9*z), (z_k - 0.9*z))
                # print(u_k)
                Y = (x_k.T @ Q @ x_k) + (u_k.T @ R @ u_k)
            else:
                # print((x_k.T @ Q @ x_k) + (u_k.T @ R @ u_k))
                phi = np.c_[phi, np.kron((z_k - 0.9*z), (z_k - 0.9*z))]
            ...
            if episode > 36:
                ...
                Y = np.delete(Y, 0, 1)
                print(H_new.shape)
                print(np.linalg.norm(H_new-H.reshape(64,1)))
                if(np.linalg.norm(H_new-H.reshape(64,1)) < 500 and flag == 0):
                    flag = 1
                    print('---------------converged--------------')
                H_new = H_new.reshape(8,8)
                # print(H)
                H_uu = H_new[-2:, -2:]
                H_ux = H_new[-2:,:6]
                # print(H_uu)
