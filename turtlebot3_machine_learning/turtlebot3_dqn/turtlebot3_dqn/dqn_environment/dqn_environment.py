...
from oelp_interfaces.srv import Opt
...
class DQNEnvironment(Node):
    def __init__(self):
        ...
        ************************************************************"""
        ...
        self.curr_state_server = self.create_service(Dqn, 'curr_state', self.current_state)
    ...
    *******************************************************************************"""
    ...
    def current_state(self, request, response):
        response.state = [self.last_pose_x, self.last_pose_y, self.last_pose_theta]
    ...
    def get_state(self):
        ...
        for i in range(24):
            # print('lmaoo -----', self.scan_ranges[i])
            self.scan_ranges[i] = max(self.scan_ranges[i], -34028229999999999496011500909022412800.000000)
            self.scan_ranges[i] = min(self.scan_ranges[i], 34028229999999999496011500909022412800.000000)
        ...
        # print(f'lmao --- {len(self.scan_ranges)}')
        self.local_step += 1
    ...
    def get_reward(self, action):
        ...
        distance_reward = (2 * self.init_goal_distance) / \
            (self.init_goal_distance + 2*self.goal_distance) - 1
